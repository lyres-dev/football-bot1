...
⚠️ Прогноз носит информационный характер.""",
    ...
    def _format_elo_analysis(self, elo_data: dict, value_bets: list,
                              home: str, away: str) -> str:
        if not elo_data:
            return ""

        lines = ["\n=== МАТЕМАТИЧЕСКИЙ АНАЛИЗ (ЭЛО + ПУАССОН) ==="]
        lines.append(f"Рейтинг Эло: {home} ({elo_data['elo_home']}) vs {away} ({elo_data['elo_away']})")
        lines.append(f"Вероятности: {home} {elo_data['p_home_win']}% | Ничья {elo_data['p_draw']}% | {away} {elo_data['p_away_win']}%")
        lines.append(f"Ожидаемые голы: {home} {elo_data['exp_home_goals']} | {away} {elo_data['exp_away_goals']} | Тотал {elo_data['exp_total']}")
        lines.append(f"Вероятность ТБ 2.5: {elo_data['p_over_25']}%")
        lines.append(f"Вероятность обе забьют: {elo_data['p_btts']}%")

        if value_bets:
            lines.append("\n=== VALUE BETS (ЦЕННЫЕ СТАВКИ) ===")
            for vb in value_bets:
                lines.append(
                    f"{vb['rating']} {vb['label']}: коэф {vb['odds']} | "
                    f"Наша вер-ть {vb['our_prob']}% vs букмекер {vb['bookie_prob']}% | "
                    f"Value +{vb['value']}%"
                )

        return "\n".join(lines)

    async def analyze_with_elo(self, match: dict, analysis_type: str,
                                home_stats: dict = None, away_stats: dict = None,
                                elo_data: dict = None, value_bets: list = None) -> str:
        elo_text = self._format_elo_analysis(elo_data or {}, value_bets or [],
                                              match["home_team"], match["away_team"])
        prompt = self._build_prompt(match, analysis_type, home_stats, away_stats)
        if elo_text:
            prompt += elo_text + "\n\nИспользуй математические вероятности из раздела ЭЛО как основу прогноза. Обязательно упомяни Value Bet если есть."
            ...
            "messages": [
                {
                    ...
                    "content": (
                        "Ты профессиональный футбольный аналитик. Пиши на русском языке. "
                        "Используй эмодзи для структуры. Давай чёткие конкретные прогнозы. "
                        "Опирайся на математические вероятности (Эло, Пуассон) и реальную статистику. "
                        "Всегда указывай точные вероятности в процентах."
                    )
                },
                {"role": "user", "content": prompt}
            ],
